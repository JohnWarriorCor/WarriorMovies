...
export class PeliculaeditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.pelicula.img === null || this.pelicula.img === '') {
      ...
      this.defaultImgUrl =
        // tslint:disable-next-line:max-line-length
        'https://firebasestorage.googleapis.com/v0/b/johnwarriormovies.appspot.com/o/prueba.jpg?alt=media&token=f38ea2dc-9710-4926-ba0f-7f6db6242290';
  ...
  guardar() {
    if (
      this.pelicula.titulo !== this.war ||
      this.pelicula.titulo !== this.war
    ) {
      this.error = false;
      this.modalReference.close();
      if (this.id === 'nuevo') {
        this.peliculaServices.nuevoPelicula(this.pelicula).subscribe(
          (data) => {
            this.showSuccess();
        ...
        this.modalReference.close();
        this.peliculaServices
          .actualizarPelicula(this.pelicula, this.id)
          .subscribe(
            (data) => {
              this.showInfo();
              this.router.navigate(['/inicio']);
